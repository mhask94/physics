...
namespace phys
{
    ...
    {
        m_velocity = (Vec3::abs(m_velocity).getX() < .0000000001f) ? m_velocity * Vec3{0,1,1} : m_velocity;
        m_velocity = (Vec3::abs(m_velocity).getY() < .0000000001f) ? m_velocity * Vec3{1,0,1} : m_velocity;
        m_velocity = (Vec3::abs(m_velocity).getZ() < .0000000001f) ? m_velocity * Vec3{1,1,0} : m_velocity;
    }

    void Sphere::update(float dt,Vec3 acceleration,Boundary* box)
    {
        Vec3 zero{0,0,0};
//        if (Vec3::isNear(acceleration,zero,0.0000001f) && this->isNearWall(box))
//            m_velocity = acceleration;
//        else
        m_velocity = m_velocity + acceleration*dt;
        if (Vec3::anyElementIsNear(m_velocity,zero,.0000000001f) && this->isNearWall(box))
